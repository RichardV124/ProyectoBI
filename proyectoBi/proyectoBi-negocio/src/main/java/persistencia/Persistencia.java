...
public class Persistencia implements Serializable {
	...
	public void registrarDetalleVenta(List<DetalleVenta> detalles, Venta venta) {
		...
		String sql = "INSERT INTO DETALLE_VENTA (ID_VENTA, " + "ID_PRODUCTO, CANTIDAD) VALUES (?1,?2,?3)";
	...
	public void crearAccesoTipoUsuario(AccesoTipoUsuario accesoTipo) {
		...
		// AccesoTipoUsuario atu = (AccesoTipoUsuario)
		// buscar(AccesoTipoUsuario.class,atuPK);
		// if (atu == null) {
	...
	public void eliminarAccesoTipoUsuario(AccesoTipoUsuario accesoTipo) {
		...
		Query q = emP.createNativeQuery(
				"DELETE FROM ACCESO_TIPO_USUARIO ac WHERE ac.ID_TIPO_USUARIO=?1 AND ac.ID_ACCESO=?2");
	...
	public List<DimensionUser> listaUsuarios() {
		...
		List<Object[]> usersBd = listarWiki(
				"SELECT CAST(u.user_name AS CHAR) username, CAST(u.user_real_name AS CHAR) NAME FROM USER u;");
	...
	public List<DimensionPage> listaPaginas() throws Exception {
		...
		List<Object[]> pagesBd = listarWiki(
				"SELECT (SELECT DATE_FORMAT(CAST(rc.rc_timestamp AS CHAR), '%d/%m/%Y')) AS fecha,"
	...
	public List<Object[]> listaTextos() {
		List<Object[]> textsBd = listarWiki(
				"SELECT CAST(si_title AS CHAR) title, CAST(si_text AS CHAR) TEXT FROM searchindex;");
	...
	public List<HechoRecentChange> listaCambios(List<DimensionUser> users, List<DimensionPage> pages) throws Exception {
		...
		List<Object[]> changesBd = listarWiki(
				"SELECT (SELECT DATE_FORMAT(CAST(rc.rc_timestamp AS CHAR), '%d/%m/%Y')) AS fecha"
						+ ", CAST(rc.rc_user_text AS CHAR) USER, CAST(rc.rc_title AS CHAR) title"
